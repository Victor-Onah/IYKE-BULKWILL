...
app.prepare().then(async () => {
    try {
        ...
        let server = (0, express_1.default)(), PORT = parseInt('3000' || process.env.PORT);
        ...
        server.get('/api/listings', authorize, async (req, res, next) => {
            try {
                ...
                res.json(await app_1.default.listings(Number(page), category ? category : undefined));
            }
            catch (error) {
                console.error(error);
                res.status(500).send();
        ...
        server.listen(PORT, () => console.log(`App is active and running on PORT: ${PORT}`));
    }
    catch (error) {
        console.log(error);
    }
